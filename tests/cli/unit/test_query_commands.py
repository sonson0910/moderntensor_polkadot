...
class TestSubnetQuery:
    ...
    def test_subnet_with_uid(self):
        ...
        result = self.runner.invoke(
            cli, ["query", "subnet", "--subnet-uid", "1", "--network", "testnet"]
        )
    ...
    def test_list_subnets(self):
        ...
        result = self.runner.invoke(cli, ["query", "list-subnets", "--network", "testnet"])
        # May fail if network is not available
        assert result.exit_code in [0, 1]
...
class TestQueryNetworkOptions:
    ...
    def test_query_with_local(self):
        ...
        result = self.runner.invoke(cli, ["query", "list-subnets", "--network", "local"])
        # May fail if network is not available
        assert result.exit_code in [0, 1]
