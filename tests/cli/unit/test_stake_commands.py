...
class TestStakeAdd:
    ...
    def test_add_requires_coldkey(self):
        ...
        result = self.runner.invoke(
            cli, ["stake", "add", "--hotkey", "test_hk", "--amount", "10000"]
        )
    ...
    def test_add_requires_amount(self):
        ...
        result = self.runner.invoke(
            cli, ["stake", "add", "--coldkey", "test_ck", "--hotkey", "test_hk"]
        )
...
class TestStakeList:
    ...
    def test_list_basic(self):
        ...
        result = self.runner.invoke(cli, ["stake", "list", "--network", "testnet"])
        # May fail if network unavailable
        assert result.exit_code in [0, 1]

    def test_list_with_limit(self):
        """Test stake list with custom limit."""
        result = self.runner.invoke(cli, ["stake", "list", "--network", "testnet", "--limit", "10"])
        # May fail if network unavailable
        assert result.exit_code in [0, 1]
