...
class TestCLIConfig:
    ...
    def test_config_from_dict(self):
        ...
        test_config_dict = {
            "network": {"name": "testnet", "rpc_url": "https://test.example.com", "chain_id": 2},
            "verbosity": 2,
        }
...
class TestCLIConfigIntegration:
    ...
    def test_cli_with_config_option(self):
        ...
        # Create test config
        test_config = {"network": {"name": "testnet", "rpc_url": "https://test.example.com"}}

        with open(self.config_path, "w") as f:
            yaml.dump(test_config, f)

        # Run CLI command with config
        result = self.runner.invoke(cli, ["--config", str(self.config_path), "utils", "version"])
    ...
    def test_network_option_override(self):
        ...
        result = self.runner.invoke(cli, ["query", "list-subnets", "--network", "mainnet"])
