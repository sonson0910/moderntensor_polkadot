...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not self.security_manager.verify_api_key(uid, api_key):
            client_ip = request.client.host if request.client else "unknown"
            logger.warning(f"Invalid API key from {client_ip} for UID {uid}")
            self.security_manager.record_failed_auth(client_ip)
            return JSONResponse(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        client_ip = request.client.host if request.client else "unknown"
        self.security_manager.reset_failed_auth(client_ip)
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.error(
                f"Error: {str(e)} for {request.method} {request.url.path} " f"({duration:.3f}s)"
            )
            raise
