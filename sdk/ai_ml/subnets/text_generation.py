...
class TextGenerationSubnet(BaseSubnet):
    ...
    def _setup_generation_model(self):
        ...
        try:
            ...
            logger.warning("transformers not installed, using fallback text generator")
            self.model = None
            self.tokenizer = None
    ...
    def _fallback_generation(self, prompt: str, max_length: int) -> str:
        """Template-based text generation (fallback when no LLM available)."""
        # Generates contextually relevant text using templates
        responses = [
